...
function ProjectService($http, WebConfig) {
    ...
    function GetAll() {
        return $http.get(WebConfig.apiconstrutec + '/proyecto/Get').then(handleSuccess, handleError('Error obteniendo todos los Proyectos'));
    }

    //Getting Projects by username
    function GetByUsername(username) {
        return $http.get(WebConfig.apiconstrutec + '/Obra/?pUse='+username).then(handleSuccess, handleError('Error Obteniendo Proyectos por Usuario'))
    }
