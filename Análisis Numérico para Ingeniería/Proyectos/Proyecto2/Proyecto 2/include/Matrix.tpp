...
namespace anpi
{
  ...
  void Matrix<T,Alloc>::fill(const T val) {
      ...
      anpi::simd::fill<T>(*this, val);


  }
    template<typename T,class Alloc>
    void Matrix<T,Alloc>::slowfill(const T val) {
  ...
  Matrix<T,Alloc> operator*(const Matrix<T,Alloc>& a,
    ...
    if(a.cols()!=b.rows()){
      throw anpi::Exception("The number of columns in A must be equal to the number of rows in B");
    }
  ...
  template<typename T,class Alloc>
  std::vector< typename Matrix<T,Alloc>::value_type > operator*(const Matrix<T,Alloc>& a,
  							const std::vector< typename Matrix<T,Alloc>::value_type >& b){

    if(a.cols()!=b.size()){
      throw anpi::Exception("The number of columns in A must be equal to the number of elements in B");
    }

  	std::vector<typename Matrix<T,Alloc>::value_type> c;
  	::anpi::fallback::product(a,b,c);
  	return c;
  }
