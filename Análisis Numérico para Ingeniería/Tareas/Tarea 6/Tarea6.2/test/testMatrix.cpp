...
void testArithmetic() {
...

  {
    ...
    try {
    	c=a*b;
        BOOST_CHECK_MESSAGE(false,"Unsupported matrix not properly catched");
    }
    catch(anpi::Exception& exc){
    	BOOST_CHECK_MESSAGE(true,"Unsupported matrix properly detected");
    }
  ...
  {
    M a = { {1,2,3},{ 4, 5, 6} };
    std::vector<typename M::value_type> b = { 7,8,9,10 };
    std::vector<typename M::value_type> r = { 50,122 };
    
    std::vector<typename M::value_type> c;
    
    try {
    	c=a*b;
        BOOST_CHECK_MESSAGE(false,"Unsupported matrix or vector not properly catched");
    }
    catch(anpi::Exception& exc){
    	BOOST_CHECK_MESSAGE(true,"Unsupported matrix or vector properly detected");
    }
