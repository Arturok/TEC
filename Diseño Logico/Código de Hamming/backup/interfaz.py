...
def fill_table2():
    ...
    fill_ErrorRecibido(1)

    fill_p1Error(binValue,1)
    fill_p2Error(binValue,1)
    fill_p3Error(binValue,1)
    fill_p4Error(binValue,1)
    fill_ConError(binValue2,1)

    R_paridad.config(text = calculoParidad(hamming))


    for i in range(16):
        hamming2 += segundaTabla[i+80].get()
...
def fill_p1(binary,par):
    ...
    for i in range(16):
        if not (i in ignorar):
            primerTabla[i+16].insert(END,newValor[index])
            index+=1
...
def fill_p1Error(binary,par):
    ...
    for i in range(16):
        if not (i in ignorar):
            segundaTabla[i+16].insert(END,newValor[index])
            index+=1



def fill_p2(binary,par):
    ignorar=[0,3,4,7,8,11,12,15]
    newValor=llenarP2(binary,par)
    index=0
    print("VAlOR: ",newValor)
    for i in range(16):
        if not(i in ignorar):
            primerTabla[i+32].insert(END,newValor[index])
...
def fill_p2Error(binary,par):
    ...
    for i in range(16):
        if not(i in ignorar):
            segundaTabla[i+32].insert(END,newValor[index])
            #print("index: "+str(index))
            index+=1
...
def fill_p3(binary,par):
    ...
    for i in range(16):
        if (i in ignorar):
            primerTabla[i+48].insert(END,newValor[index])
...
def fill_p3Error(binary,par):
    ...
    for i in range(16):
        if (i in ignorar):
            segundaTabla[i+48].insert(END,newValor[index])
...
def fill_p4(binary,par):
    ...
    for i in range(16):
        if (i in ignorar):
            primerTabla[i+64].insert(END,newValor[index])
            #print("index: "+str(index))
            index+=1
...
def fill_p4Error(binary,par):
    ...
    for i in range(16):
        if (i in ignorar):
            segundaTabla[i+64].insert(END,newValor[index])
            #print("index: "+str(index))
            index+=1
...
def fill_ConParidad(hexa,par):
    ...
    print("c: " + c)
    for i in range(16):
        primerTabla[i+80].insert(END,c[i])
...
def fill_ConError(bin,par):
    ...
    print("c2: " + c)
    for i in range(16):
        segundaTabla[i+80].insert(END,c[i])

def fill_ErrorRecibido(par):
    for i in range(16):
        segundaTabla[i].insert(END,primerTabla[i+80].get())
...
Dato_conParidad.place(x=70,y=375)
posy=275
for i in range(4):
...
Dato_conParidad =  Label(C_root, width=10, font=("Ubuntu Medium",12),justify='center',text = "Con Paridad:",bg="white",fg="black")
Dato_conParidad.place(x=70,y=600)
posy=500
for i in range(4):
    p=Label(C_root, width=10, font=("Ubuntu Medium",12),justify='center',text = "p"+str(i+1),bg="white",fg="black")
    p.place(x=70,y=posy+25*i)
